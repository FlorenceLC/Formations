/**
 * pages.js — Logique de rendu de chaque page (tout async/await)
 */

/* ================================================================
   UTILITAIRE : wrapper async avec gestion d'erreur + loading
================================================================ */
...
const Pages = {
  ...
  dashboard: {
    ...
    async _render() {
      ...
      document.getElementById('dashboard-greeting').textContent = `Bonjour, ${u.prenom} 👋`;

      const [stats, allFormations] = await Promise.all([
        DB.getUserStats(u.id),
        DB.getFormations(),
      ]);

      const now = new Date().toISOString();
      const aVenir = allFormations.filter(f => f.dateDebut >= now && f.statut !== 'annulee');
      ...
      const myFormations = await DB.getUserFormations(u.id);
      const myIds = new Set(myFormations.map(f => f.id));
      ...
      container.innerHTML = aVenir.slice(0, 6).map(f => {
        const isInscrit = myIds.has(f.id);
        ...
        return `
          <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border);">
            <div style="background:${isInscrit ? 'var(--primary-light)' : 'var(--bg)'};border-radius:8px;padding:8px 12px;text-align:center;min-width:52px;flex-shrink:0;">
              <div style="font-size:18px;font-weight:800;color:${isInscrit ? 'var(--primary)' : 'var(--secondary)'};">${d.getDate()}</div>
              <div style="font-size:10px;font-weight:700;color:var(--text-muted);text-transform:uppercase;">${d.toLocaleDateString('fr-FR',{month:'short'})}</div>
            </div>
            <div style="flex:1;min-width:0;">
              <div style="font-weight:700;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${cat?.nom || '—'}</div>
  ...
  planning: {
    ...
    async _render() {
      ...
      const myFormations = await DB.getUserFormations(App.currentUser.id);
      const myIds = new Set(myFormations.map(f => f.id));
      const cats = await DB.getCategories();
      const container = document.getElementById('planning-view');
      if (this.view === 'month') this._renderMonth(container, formations, myIds, cats);
      else                       this._renderWeek(container, formations, myIds, cats);
    },
    ...
    _renderMonth(container, formations, myIds, cats) {
      ...
      for (let i = 0; i < 42; i++) {
        ...
        html += `<div class="cal-cell${!isCurrent ? ' other-month' : ''}${isToday ? ' today' : ''}">
          <div class="day-num">${cellDate.getDate()}</div>`;

        dayForms.slice(0, 3).forEach(f => {
          const isMine   = myIds.has(f.id);
          const catName  = cats.find(c => c.id === f.categorieId)?.nom || '—';
          html += `<div class="cal-event${isMine ? ' my-event' : ''}"
            title="${catName}\n👨‍🏫 ${f.formateurs||'—'}\n📍 ${f.lieu||'—'}\n👥 ${f.inscritsCount}/${f.placesMax}"
            onclick="Pages.formations._openDetail('${f.id}')">
        ...
        if (dayForms.length > 3)
          html += `<div class="cal-more" onclick="Pages.formations._openDetail('${dayForms[3].id}')">+${dayForms.length - 3} autre(s)</div>`;
    ...
    _renderWeek(container, formations, myIds, cats) {
      const ws = this._weekStart(this.currentDate);
      const today = new Date(); today.setHours(0,0,0,0);
      ...
      for (let i = 0; i < 7; i++) {
        ...
        html += `<div class="week-col${isToday ? ' today' : ''} show">
          <div class="week-col-header"><div class="wday">${jours[i].slice(0,3)}</div><div class="wdate">${d.getDate()}</div></div>
        ...
        if (!dayForms.length) {
          ...
          dayForms.forEach(f => {
            const isMine  = myIds.has(f.id);
            const catName = cats.find(c => c.id === f.categorieId)?.nom || '—';
            html += `<div class="week-event${isMine ? ' my-event' : ''}" onclick="Pages.formations._openDetail('${f.id}')">
    ...
    prev()  { if (this.view==='month') this.currentDate.setMonth(this.currentDate.getMonth()-1); else this.currentDate.setDate(this.currentDate.getDate()-7); this.render(); },
    next()  { if (this.view==='month') this.currentDate.setMonth(this.currentDate.getMonth()+1); else this.currentDate.setDate(this.currentDate.getDate()+7); this.render(); },
  ...
  formations: {
    ...
    async _renderTable() {
      ...
      tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--text-muted);">⏳ Chargement…</td></tr>`;
      const [formations, cats] = await Promise.all([DB.getFormations(this.filters), DB.getCategories()]);
      const u = App.currentUser;
      ...
      // Récupérer les inscriptions de l'utilisateur courant une seule fois
      const myFormations = await DB.getUserFormations(u.id);
      const myIds = new Set(myFormations.map(f => f.id));
    ...
    async _openDetail(id) {
      run(async () => {
        const [f, cats, inscriptions] = await Promise.all([
          DB.getFormationById(id),
          DB.getCategories(),
          DB.getInscriptions(id),
        ]);
        if (!f) return;
        const cat = cats.find(c => c.id === f.categorieId);
        const isInscrit = await DB.isInscrit(id, App.currentUser.id);
        ...
        document.getElementById('detail-modal-body').innerHTML = `
          ...
          <div class="detail-row"><span class="detail-icon">👥</span><div><div class="detail-key">Participants (${f.inscritsCount}/${f.placesMax})</div>
            <div class="participants-list">${inscriptions.length
              ? inscriptions.map(i => { const usr=i.user; return `<span class="participant-chip" style="background:${usr?.couleur||'#64748b'}22;color:${usr?.couleur||'#64748b'};border-color:${usr?.couleur||'#64748b'}44;">${usr?.prenom||''} ${usr?.nom||''}</span>`; }).join('')
              : '<span style="color:var(--text-muted);font-size:12px;">Aucun inscrit</span>'}</div>
          </div></div>
        ...
        if (f.statut !== 'annulee') {
          if (isInscrit) {
            const b = document.createElement('button'); b.className='btn btn-danger'; b.textContent='Se désinscrire';
            b.onclick = () => { Pages.formations._desinscrire(id); Modal.close('detail-modal'); };
            footer.appendChild(b);
          } else {
            const b = document.createElement('button'); b.className='btn btn-success'; b.textContent="S'inscrire";
            b.onclick = () => { Pages.formations._inscrire(id); Modal.close('detail-modal'); };
            footer.appendChild(b);
          }
        }
    ...
    _inscrire(id) {
      run(async () => {
        const f = await DB.getFormationById(id);
        if (!f) return;
        if (f.inscritsCount >= f.placesMax) { toast('Formation complète', 'error'); return; }
        await DB.inscrire(id, App.currentUser.id);
    ...
    _desinscrire(id) {
      confirmDialog('Se désinscrire', 'Confirmer la désinscription ?', () => {
        run(async () => {
          await DB.desinscrire(id, App.currentUser.id);
    ...
    _delete(id) {
      run(async () => {
        const f = await DB.getFormationById(id);
        const cats = await DB.getCategories();
        const catName = cats.find(c => c.id===f?.categorieId)?.nom || 'cette formation';
        confirmDialog('Supprimer', `Supprimer « ${catName} » du ${Fmt.date(f?.dateDebut)} ?`, () => {
          run(async () => { await DB.deleteFormation(id); toast('Formation supprimée','info'); this._renderTable(); });
        });
      });
    },
    ...
    _duplicate(id) {
      run(async () => {
        await DB.duplicateFormation(id, App.currentUser.id);
    ...
    async _openForm(id = null) {
      const isEdit = !!id;
      const [f, cats, users, catalogue] = await Promise.all([
      ...
      document.getElementById('form-modal-title').textContent = isEdit ? '✏️  Modifier' : '➕  Nouvelle formation';
      ...
      catalogue.forEach(c => { const o=document.createElement('option'); o.value=c.id; o.textContent=c.titre||c.nom||''; catSel.appendChild(o); });
      ...
      // Formateurs (liste des utilisateurs FTSI)
      const formateursSel = document.getElementById('form-formateurs');
      formateursSel.innerHTML = '';
      users.filter(u => !u.isConfigurateur && !u.is_configurateur).forEach(u => {
        const o = document.createElement('option');
        const nom = `${u.prenom||u.prenom||''} ${u.nom||''}`.trim();
      ...
      Array.from(formateursSel.options).forEach(o => { o.selected = existing.includes(o.value); });
    ...
    save() {
      run(async () => {
        ...
        const existingF  = id ? await DB.getFormationById(id) : null;
        ...
        await DB.saveFormation({
          ...
          createdBy:   existingF?.createdBy || App.currentUser.id,
        });
  ...
  catalogue: {
    render() { run(() => this._render()); },
    async _render() {
      const [items, cats] = await Promise.all([DB.getCatalogue(), DB.getCategories()]);
      const tbody = document.getElementById('catalogue-tbody');
      if (!items.length) {
        tbody.innerHTML = `<tr><td colspan="3" style="text-align:center;padding:32px;color:var(--text-muted);">Catalogue vide</td></tr>`;
        return;
      }
      tbody.innerHTML = items.map(item => {
        const cat = cats.find(c=>c.id===(item.categorie_id||item.categorieId));
        return `<tr>
          ...
          <td>${(item.duree_heures||item.dureeHeures)?((item.duree_heures||item.dureeHeures)+' h'):'—'}</td>
        </tr>`;
      }).join('');
    },
  },
  ...
  admin: {
    ...
    showTab(tab) {
      if (tab==='config' && !App.currentUser.isConfigurateur) { toast('Accès réservé au configurateur','error'); tab='users'; }
      this._tab = tab;
      ...
      if (tab==='users')  run(()=>this._renderUsers());
      ...
      tbody.innerHTML = users.map(u => `<tr>
        <td>
          <div style="display:flex;align-items:center;gap:8px;">
            <div class="sidebar-avatar" style="background:${u.couleur||'#64748b'};width:28px;height:28px;font-size:11px;">${((u.prenom||'')[0]||'').toUpperCase()+((u.nom||'')[0]||'').toUpperCase()}</div>
            <div><div style="font-weight:600;">${u.prenom||''} ${u.nom||''}</div><div style="font-size:11px;color:var(--text-muted);">@${u.username}</div></div>
          </div>
        </td>
        <td>${u.email||'—'}</td>
        <td>${(u.isConfigurateur||u.is_configurateur)?'<span class="badge badge-gray" style="background:#0F172A22;color:#0F172A;">🔧 Configurateur</span>':(u.isAdmin||u.is_admin?'<span class="badge badge-warning">⭐ Admin</span>':'<span class="badge badge-gray">Collaborateur</span>')}</td>
        <td>
          <div style="display:flex;gap:4px;">
            ...
            ${u.id!==App.currentUser.id&&!(u.isConfigurateur||u.is_configurateur)?`<button class="btn btn-sm btn-icon" style="background:var(--danger-light);color:var(--danger);" title="Supprimer" onclick="Pages.admin._deleteUser('${u.id}')">🗑</button>`:''}
          </div>
        </td>
      </tr>`).join('');
    },

    async _openUserForm(id=null) {
      ...
      document.getElementById('user-is-admin').checked = u?.isAdmin||u?.is_admin||false;
    ...
    _editUser(id)   { run(()=>this._openUserForm(id)); },
    _deleteUser(id) {
      run(async () => {
        const u = await DB.getUserById(id);
        confirmDialog('Supprimer',`Supprimer « ${u?.prenom} ${u?.nom} » ?`, ()=>{
          run(async ()=>{ await DB.deleteUser(id); toast('Utilisateur supprimé','info'); this._renderUsers(); });
        });
      });
    },
    ...
    saveUser() {
      run(async () => {
        ...
        if (!id && !password) { toast('Mot de passe obligatoire','error'); return; }

        const oldUser  = id ? await DB.getUserById(id) : null;
        const pwdHash  = password ? hashPassword(password) : oldUser?.password_hash||oldUser?.passwordHash;
        ...
        await DB.saveUser({
          ...
          isConfigurateur: false, // jamais via ce formulaire
          isActive:        true,
    ...
    changePwd() {
      const id  = document.getElementById('user-id').value;
      const pwd = prompt('Nouveau mot de passe (minimum 4 caractères) :');
      if (!pwd) return;
      if (pwd.length < 4) { toast('Mot de passe trop court','error'); return; }
      run(async ()=>{ await DB.changePassword(id, pwd); toast('Mot de passe modifié ✅','success'); });
    },
    ...
    _renderConfig() {
      ...
      status.className = cfg.url&&cfg.anonKey ? 'config-status connected' : 'config-status disconnected';
      status.innerHTML = cfg.url&&cfg.anonKey ? '✅ Configuration Supabase enregistrée' : '⚠️ Non configuré';
    },
    ...
    testConnection() {
      run(async ()=>{
        const cfg = DB.getSupabaseConfig();
        if (!cfg.url||!cfg.anonKey) { toast('Complétez l\'URL et la clé','error'); return; }
        toast('Test en cours…','info');
        const ok = await DB.ping().catch(()=>false);
        if (ok) toast('✅ Connexion Supabase réussie !','success');
        else    toast('❌ Connexion impossible — vérifiez URL, clé anon et le schéma SQL','error');
      });
    },
  },
  ...
  profil: {
    ...
    async _render() {
      ...
      avatar.style.background = u.couleur;
      document.getElementById('profil-fullname').textContent       = `${u.prenom} ${u.nom}`;
      document.getElementById('profil-username-display').textContent = `@${u.username}`;
    },
    ...
    changePwd() {
      run(async ()=>{
        ...
        const check    = await DB.authenticate(App.currentUser.username, current);
        if (!check)         { toast('Mot de passe actuel incorrect','error'); return; }
        ...
        await DB.changePassword(App.currentUser.id, newPwd);
        ['profil-pwd-current','profil-pwd-new','profil-pwd-confirm'].forEach(id=>document.getElementById(id).value='');
